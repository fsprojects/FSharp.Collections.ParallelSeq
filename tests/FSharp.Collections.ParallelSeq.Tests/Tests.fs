...
let VerifyPSeqsEqual seq1 seq2 =
    ...
    if len1 <> len2 then
        Assert.Fail(sprintf "seqs not equal length: %d and %d" len1 len2)
    else
        let set1 = set seq1
        let set2 = set seq2
...
[<Test>]
let TestMapWithException () = MapWithExceptionTester PSeq.map
...
[<Test>]
let TestSingletonCollectWithException () = MapWithExceptionTester(fun f -> PSeq.collect (f >> PSeq.singleton))
...
//        Assert.AreEqual("E",resultStr)
    ...
    CheckThrowsArgumentNullException (fun () ->
        PSeq.reduce (fun (x: string) (y: string) -> x.Remove(0, y.Length)) nullSeq |> ignore)
